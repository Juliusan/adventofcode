...
read_register(Line, Name) ->
    ...
    integer_to_bits(Value).


integer_to_bits(Int) -> integer_to_bits(Int, []).
...
bits_to_integer([Bit|Bits], Acc) -> bits_to_integer(Bits, Acc*2+Bit).
...
program_to_map([OpCode,Operand|Program], Address, Acc) ->
    ...
    NewAcc = Acc#{Address => {Op, integer_to_bits(Operand)}},
...
xor_lists(Bits1, Bits2) -> xor_lists(Bits1, Bits2, []).
xor_lists([],           [],           Acc) -> lists:reverse(Acc);
xor_lists([Bit1|Bits1], [Bit2|Bits2], Acc) -> xor_lists(Bits1, Bits2, [xor_bits(Bit1, Bit2) | Acc]);
xor_lists([],           [Bit2|Bits2], Acc) -> xor_lists([],    Bits2, [Bit2                 | Acc]);
xor_lists([Bit1|Bits1], [],           Acc) -> xor_lists(Bits1, [],    [Bit1                 | Acc]).
...
run(A, B, C, Address, Program, Output) ->
    ...
    DvFun = fun(OperandCombo) ->
        ...
        OperandInt = bits_to_integer(Operand),
    ...
    case maps:get(Address, Program, undefined) of
        undefined ->
            utils:print("FINAL: A=~p, B=~p, C=~p, Address=~p", [bits_to_integer(A), bits_to_integer(B), bits_to_integer(C), Address]),
            {A, Output};
        {adv, OperandCombo} ->
            Result = DvFun(OperandCombo),
            run(Result, B, C, Address+2, Program, Output);
        {bxl, Operand} ->
            Result = xor_lists(B, Operand),
        ...
        {bxc, _Operand} ->
            Result = xor_lists(B, C),
            run(A, Result, C, Address+2, Program, Output);
        {out, OperandCombo} ->
            Operand = combo_operand(OperandCombo, A, B, C),
            Result = lists:sublist(Operand, 3),
            run(A, B, C, Address+2, Program, [bits_to_integer(Result)|Output]);
        {bdv, OperandCombo} ->
            Result = DvFun(OperandCombo),
            run(A, Result, C, Address+2, Program, Output);
        {cdv, OperandCombo} ->
...
trace_back([Out|OtherOuts]=Outs, Index, Acc) ->
    ...
    case {Digit1, Digit2, Digit3} of
        ...
        {_, _, _        } ->
            Digit1I = invert(Digit1),
            Shift = bits_to_integer([Digit1I, Digit2, Digit3]),
            ...
            case {C1, C2, C3} of
                ...
                {_, _, _        } ->
                    Result1 =        xor_bits(Digit1I, C1),
                    Result2 = invert(xor_bits(Digit2,  C2)),
                    Result3 = invert(xor_bits(Digit3,  C3)),
                    Result = bits_to_integer([Result1, Result2, Result3]),
...
solve_2(FileName) ->
    ...
    % Result check
    Program = program_to_map(ProgramList, 0, #{}),
    {[], ProgramListR} = run(integer_to_bits(Int), B, C, 0, Program, []),
