...
get_hand_type(Cards) ->
    ...
    [Jokers | OtherCards] = erlang:tuple_to_list(CardsCount),
    CardsCountNoZeros = lists:filter(fun(D) -> D =/= 0 end, OtherCards),
    CardsCountSorted = lists:reverse(lists:sort(CardsCountNoZeros)),
    CardsCountWithJokers = case CardsCountSorted of
        []            -> [Jokers];
        [High | Rest] -> [High + Jokers | Rest]
    end,
    case CardsCountWithJokers of
        [5]             -> 7;
        [4 | _]         -> 6;
        [3, 2 ]         -> 5;
        [3 | _]         -> 4;
...
card_str_to_card($T) -> 10;
card_str_to_card(D) when $2 =< D, D =< $9 -> D-$0;
card_str_to_card($J) ->  1.
